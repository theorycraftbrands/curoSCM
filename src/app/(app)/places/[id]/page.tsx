...
export default async function LocationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: location } = await supabase.from("locations").select("*, business:businesses(id, name)").eq("id", id).single();
  ...
  const address = [location.address_line_1, location.address_line_2, [location.city, location.state_province].filter(Boolean).join(", "), location.postal_code, location.country].filter(Boolean).join("\n");
        ...
        <div className="rounded-xl border bg-card p-6 shadow-sm">
          ...
          {address ? (
            <div className="flex items-start gap-2">
              ...
              <pre className="text-sm whitespace-pre-line font-sans">{address}</pre>
            </div>
          ) : (
